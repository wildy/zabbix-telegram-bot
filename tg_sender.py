...
%s
    ...
    # Do not show notifications if notification level is set low for this
    # event (by config or '-l' parameter)
    if level < -9:
    ...
    if status == 'PROBLEM':
        ack_button = types.InlineKeyboardButton(
            text="Acknowledge", callback_data="ACKNOWLEDGE %s" % event_id)
        kb.add(ack_button)

    bot.send_message(group_id, message, parse_mode='HTML',
                     disable_notification=disable_notification, reply_markup=kb)

parser = argparse.ArgumentParser(add_help=True)
parser.add_argument('-i', dest='event_id',
                    help='Event ID (for inline acknowledge)', required=True, type=int)
...
parser.add_argument('-g', dest='group',
                    help='Group to send trigger to', required=True)
parser.add_argument('-l', dest='level',
                    help='Override this event notification level: -2 = do not send at all; -1 = send without sound, 0 = send standard notification', type=int, default=None)

args = parser.parse_args()

handle_trigger(args.event_id, args.trigger, args.severity,
               args.status, args.group, zabbix_description, args.level)